...
//
// AIDEV-NOTE: 본 파일이 CJS 인 것은 valkey-store 만의 제약이 아니라 CHP 의
// storage backend 컨벤션이다. `lib/configproxy.js:159` `loadStorage()` 가
// `require()` (createRequire 로 ESM 안에서 만든 CJS require) 로 backend 를
// 동기 로드하기 때문에 모든 backend 가 CJS 여야 한다 (upstream 의
// `test/dummy-store.cjs` 도 동일). loadStorage 를 async + 동적 import 로
// 바꾸면 ESM backend 도 가능하지만, 호출 체인의 ConfigurableProxy 생성자
// async 화 + 기존 CJS backend 와의 호환 유지 비용이 커서 현 컨벤션을 유지.
// 그 결과 ESM 인 lib/trie.js 는 동적 `import()` 로 가져오고, 동기 메서드가
// 필요한 trimPrefix 는 본 파일에 복제했다.
//
