...
export class MemoryStore extends BaseStore {
  ...
  update(path, data) {
    ...
    if (this.routes[cleaned]) Object.assign(this.routes[cleaned], data);
...
export class ValkeyStore extends BaseStore {
  constructor(options = {}) {
    ...
    // MemoryStore 의 read-after-write 성격을 흉내내기 위해 fire-and-forget 으로
    // 호출되는 write (예: res.on("finish") -> updateLastActivity) 의 Promise 를
    // 추적하고, 모든 read 는 진행 중인 write 가 끝난 뒤 실행한다.
  ...
  // 외부 (ConfigurableProxy.updateLastActivity 등) 가 fire-and-forget 으로
  // 시작한 multi-step write chain 도 inflight 에 포함시키기 위한 public hook.
  trackInflight(p) {
    this._inflightWrites.add(p);
    p.finally(() => this._inflightWrites.delete(p));
    return p;
  }
  ...
  add(path, data) {
    return this.trackInflight(this._add(path, data));
  }

  async _add(path, data) {
    await this.ready;
  ...
  update(path, data) {
    return this.trackInflight(this._update(path, data));
  }
  ...
  remove(path) {
    return this.trackInflight(this._remove(path));
  }
