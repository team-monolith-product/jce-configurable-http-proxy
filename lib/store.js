...
// VALKEY_URL 은 endpoint 만 담고 인증 정보는 분리한다 (rotation/secret 분리 정책).
// scheme `rediss://` 또는 `tls://` 면 TLS 자동 활성. URL 안에 user/pass 가
// 들어와도 무시 (helm 단계에서 분리되어야 함).
function parseValkeyEndpoint(input) {
  ...
  if (input.includes("://")) {
    ...
    host = h;
    port = p ? parseInt(p, 10) : VALKEY_DEFAULT_PORT;
  }
  return { address: { host, port }, useTLS };
}
...
export class ValkeyStore extends BaseStore {
  constructor(options = {}) {
    ...
    const { address, useTLS } = parseValkeyEndpoint(url);
    this._address = address;
    // useTLS 는 URL scheme 으로 자동 판정. options 또는 VALKEY_USE_TLS 로 강제 가능.
    this._useTLS =
      options.valkeyUseTls ??
      (process.env.VALKEY_USE_TLS ? process.env.VALKEY_USE_TLS === "true" : useTLS);

    // 인증 정보는 URL 과 분리. AWS Secrets Manager 등에서 token 만 시크릿으로
    // 주입하고 endpoint 는 plain helm value 로 두기 위함.
    const password = options.valkeyAuthToken || process.env.VALKEY_AUTH_TOKEN;
    const username = options.valkeyUsername || process.env.VALKEY_USERNAME;
    this._credentials = password ? { password, ...(username ? { username } : {}) } : undefined;
