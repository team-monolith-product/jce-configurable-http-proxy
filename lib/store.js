...
export class ValkeyStore extends BaseStore {
  constructor(options = {}) {
    ...
    const keyPrefix =
      options.valkeyKeyPrefix || process.env.VALKEY_KEY_PREFIX || "chp";
  ...
  _onInitError(err) {
    if (this.log)
      this.log.error("ValkeyStore: initialization failed: %s", err.message);
    throw err;
  }
  ...
  async add(path, data) {
    await this.ready;
    await this.client.hset(this.routesKey, {
      [this.cleanPath(path)]: JSON.stringify(data),
    });
    return null;
  }
