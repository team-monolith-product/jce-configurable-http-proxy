...
export class ConfigurableProxy extends EventEmitter {
  ...
  updateLastActivity(prefix) {
    ...
    // 사전 get 으로 존재 검사를 하지 않는다. update 가 missing key 를 silent
    // no-op 처리하므로 한 번의 호출로 충분하고, ValkeyStore 의 _drain 과
    // 재진입 deadlock 도 피한다.
    return this._routes.update(prefix, { last_activity: new Date() }).then(metricsTimerEnd);
  }
