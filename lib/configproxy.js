...
export class ConfigurableProxy extends EventEmitter {
  ...
  addRoute(path, data) {
    ...
    // ValkeyStore 같은 비동기 백엔드에서는 add() 가 resolve 된 직후 caller 가
    // _routes.get 으로 last_activity 를 검증하는 테스트가 race 로 실패한다.
    // updateLastActivity 까지 chain 으로 await 해 add → log 사이에 끼워 넣는다.
    return this._routes
      .add(path, data)
