...
export function parseListenOptions(args, logger) {
  ...
  if (args.apiSocket) {
    ...
    listen.apiPort = args.apiPort
      ? parseInt(args.apiPort)
      : listen.port
        ? listen.port + 1
        : 8001;
...
function camelCaseify(options) {
  ...
  Object.keys(options).forEach((key) => {
    const camelKey = key.replace(
      /_(.)/g,
      function (match, part, offset, string) {
...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    // ValkeyStore 같은 비동기 백엔드에서 fire-and-forget 으로 발생하는
    // last_activity 쓰기를 추적해 테스트나 graceful shutdown 시 await 가능하게 한다.
    this._activityPromises = new Set();
  ...
  addRoute(path, data) {
    ...
    return this._routes
      .add(path, data)
      .then(() => that.updateLastActivity(path))
      .then(() => {
    ...
    if (inactiveSinceParam) {
      ...
      if (isFinite(timestamp)) {
        ...
        fail(
          ...
          res,
          400,
          "Invalid datestamp '" + inactiveSinceParam + "' must be ISO8601.",
        );
        return;
      }
    }
  ...
  async targetForReq(req) {
    ...
    var path =
      basePath +
      decodeURIComponent(new URL(req.url, "http://example.com").pathname);
  ...
  updateLastActivity(prefix) {
    ...
    this._activityPromises.add(p);
    p.finally(() => this._activityPromises.delete(p));
    return p;
  }

  // 비동기 last_activity 쓰기가 모두 끝날 때까지 기다린다. 주로 테스트용.
  async flushActivity() {
    while (this._activityPromises.size > 0) {
      await Promise.allSettled([...this._activityPromises]);
    }
  }
