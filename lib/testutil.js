...
export function setupProxy(listenOptions, options, paths) {
  ...
  // 같은 spec 안에서 setupProxy 가 여러 번 호출될 때 ValkeyStore 의 default
  // keyspace 를 공유하지 않도록 매 호출마다 random prefix 부여.
  options.valkeyKeyPrefix =
    options.valkeyKeyPrefix || `chp-test-${crypto.randomBytes(4).toString("hex")}`;
